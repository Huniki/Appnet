...
class AutoDriveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # # Only Mascio Enhancement 数据增强 # 自己写的
        ...
        #     random.seed(idx)
        #     # if self.is_train and int(idx_0) == 9: # 只增强Straight or Right Turn Arrow
        ...
        #                     data_label_new = np.append(data_label_new, cropped_line, axis=0)
        ...
        # data_label = data_label_new
                ...
                # if (x1_new2 >0 and x2_new2 <1280 and y1_new2>0 and  y2_new2 <= 720):
                #     # 二次竖向重叠
                #     Is_add = True
                #     for line0 in data_label:
                #         x1_0, y1_0, x2_0, y2_0 = line0[1]*1280, line0[2]*720, line0[3]*1280, line0[4]*720
                #         if (x1_new2>x1_0 and y1_new2>y1_0 and x1_new2<x2_0 and y1_new2<y2_0) or (x2_new2>x1_0 and y2_new2>y1_0 and x2_new2<x2_0 and y2_new2<y2_0) or (x1_new2<x1_0 and y1_new2<y1_0 and x2_new2>x2_0 and y2_new2>y2_0):
                #             Is_add = False
                #             break
                #     if Is_add:
                #         try:
                #             cropped_line = [[idx_0, x1_new2/1280, y1_new2/720, x2_new2/1280, y2_new2/720]]
                #             data_label = np.append(data_label, cropped_line, axis=0)
                #             img[int(y1_new2):int(y2_new2), int(x1_new2):int(x2_new2)] = img[int(y1):int(y2), int(x1):int(x2)]
                #         except:
                #             Is_add = True

                # if (x1_new3 >0 and x2_new3 <1280 and y1_new3>0 and  y2_new3 <= 720) and (idx_0 > 1):
                #     # 一次向上竖向重叠
                #     Is_add = True
                #     for line0 in data_label:
                #         x1_0, y1_0, x2_0, y2_0 = line0[1]*1280, line0[2]*720, line0[3]*1280, line0[4]*720
                #         if (x1_new3>x1_0 and y1_new3>y1_0 and x1_new3<x2_0 and y1_new3<y2_0) or (x2_new3>x1_0 and y2_new3>y1_0 and x2_new3<x2_0 and y2_new3<y2_0) or (x1_new3<x1_0 and y1_new3<y1_0 and x2_new3>x2_0 and y2_new3>y2_0):
                #             Is_add = False
                #             break
                #     if Is_add:
                #         try:
                #             cropped_line = [[idx_0, x1_new3/1280, y1_new3/720, x2_new3/1280, y2_new3/720]]
                #             data_label = np.append(data_label, cropped_line, axis=0)
                #             img[int(y1_new3):int(y2_new3), int(x1_new3):int(x2_new3)] = img[int(y1):int(y2), int(x1):int(x2)]
                #         except:
                #             Is_add = True

                # if (x1_new4 >0 and x2_new4 <1280 and y1_new4>0 and  y2_new4 <= 720) and (idx_0 > 1):
                #     # 二次向上竖向重叠
                #     Is_add = True
                #     for line0 in data_label:
                #         x1_0, y1_0, x2_0, y2_0 = line0[1]*1280, line0[2]*720, line0[3]*1280, line0[4]*720
                #         if (x1_new4>x1_0 and y1_new4>y1_0 and x1_new4<x2_0 and y1_new4<y2_0) or (x2_new4>x1_0 and y2_new4>y1_0 and x2_new4<x2_0 and y2_new4<y2_0) or (x1_new4<x1_0 and y1_new4<y1_0 and x2_new4>x2_0 and y2_new4>y2_0):
                #             Is_add = False
                #             break
                #     if Is_add:
                #         try:
                #             cropped_line = [[idx_0, x1_new4/1280, y1_new4/720, x2_new4/1280, y2_new4/720]]
                #             data_label = np.append(data_label, cropped_line, axis=0)
                #             img[int(y1_new4):int(y2_new4), int(x1_new4):int(x2_new4)] = img[int(y1):int(y2), int(x1):int(x2)]
                #         except:
                #             Is_add = True

        # Specific Mascio Enhancement数据增强
        # cropped_path0 = '/home/ubuntu/xzy/0yolope/SDExpressway/traffic_object_cropped/'
        # f=open('/home/ubuntu/xzy/0yolope/SDExpressway/traffic_object_cropped.txt','r')
        # lines=f.readlines()
        # f.close()
        # c_c = 10
        # p = 0.9 # 数据增强概率
        ...
        # Is_Enhancement = False # 标记是否增强
        # # if self.is_train: # 限定只有训练的时候增强
        # # if True:
        # if False:
        #     random.seed(idx)
        #     if random.random() > p : # Straight or Right Turn Arrow增强
        #         Is_add = True
        ...
        #                 # # 粘贴可驾驶区域
        #                 # cropped_id = fileList[cropped_id].split('_')[1]
        #                 # if int(cropped_id)%5 == 0:
        #                 #     cropped_path = 'val/'+cropped_id+'.png'
        #                 # else:
        #                 #     cropped_path = 'train/'+cropped_id+'.png'
        #                 # mask = cv2.imread(data["mask"],0)
        #                 # mask_cropped = cv2.imread('/home/ubuntu/xzy/0yolope/SDExpressway/drivable area labels/'+cropped_path,0)
        #                 # mask[int(y1):int(y2), int(x1):int(x2)] = mask_cropped[int(y1):int(y2), int(x1):int(x2)]
        #                 # cv2.imshow("mask",mask) 
        #                 # cv2.waitKey(10000)

        #                 # # 粘贴车道线
        #                 # lane = cv2.imread(data["lane"],0)
        #                 # lane_cropped = cv2.imread('/home/ubuntu/xzy/0yolope/SDExpressway/lane line labels/'+cropped_path,0)
        #                 # lane[int(y1):int(y2), int(x1):int(x2)] = lane_cropped[int(y1):int(y2), int(x1):int(x2)]
        ...
        #                 Is_add = True
            ...
            # if random.random() > p: # Straight Ahead Arrow增强
            #     Is_add = True
            #     if id_image >= 3294 and Only_day: # 只加强白天的图片
            #         Is_add = False
            ...
            # if random.random() > p-0.1 : # Warning Sign增强
            #     Is_add = True
            #     if id_image >= 3294 and Only_day: # 只加强白天的图片
            #         Is_add = False
