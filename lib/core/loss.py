    ...
    def _forward_impl(self, predictions, targets, shapes, model):
        ...
        for i, pi in enumerate(predictions[0]):  # layer index, layer predictions
            ...
            if n:
                ...
                if model.nc > 1:  # cls loss (only if multiple classes) # 2023.08.16
...
def get_loss(cfg, device):
    ...
    """
    # class loss criteria # BCEWithLogitsLoss 二分类损失函数
    ...
    if gamma > 0:
        # BCEcls, BCEobj = FocalLoss(BCEcls, gamma), FocalLoss(BCEobj, gamma) # FocalLoss 函数
        BCEcls, BCEobj = VFLoss(BCEcls, gamma), VFLoss(BCEobj, gamma) # VariFocal Loss 函数 # 20230922
#2023
